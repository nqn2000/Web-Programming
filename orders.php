...
<head>
  ...
  <title>Baby Home Play : Orders</title>
...
<body style="background-image: url(bgBabyhomeplay3.png); background-size: 100%;">
  ...
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
        <label for="ordate" class="col-sm-3 control-label">Order Date</label>
          <div class="col-sm-9">
             <input name="orderdate" type="text" class="form-control" id="ordate" placeholder="Order Date" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
             </div>
        </div>
   ...
   <div class="form-group">
     ...
     <select name="cid" class="form-control" id="customerid" required>
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a173656_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
...
<div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a173656, tbl_staffs_a173656_pt2, tbl_customers_a173656_pt2 WHERE ";
        $sql = $sql."tbl_orders_a173656.fld_staff_num = tbl_staffs_a173656_pt2.fld_staff_num and ";
        $sql = $sql."tbl_orders_a173656.fld_customer_num = tbl_customers_a173656_pt2.fld_customer_num LIMIT $start_from, $per_page";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a173656");
            $stmt->execute();
            $result = $stmt->fetchAll();
